                                    ...
                                    const D3D12_HEAP_PROPERTIES& heapProps)
{
  ...
  CHECK_HR(m_device->CreateCommittedResource(&heapProps, D3D12_HEAP_FLAG_NONE, &bufDesc,
...
IDxcBlob* CompileShaderLibrary(LPCWSTR fileName)
{
  ...
  if (!pCompiler)
  {
      CHECK_HR(DxcCreateInstance(CLSID_DxcCompiler, __uuidof(IDxcCompiler), (void **)&pCompiler));
      CHECK_HR(DxcCreateInstance(CLSID_DxcLibrary, __uuidof(IDxcLibrary), (void **)&pLibrary));
      CHECK_HR(pLibrary->CreateIncludeHandler(&dxcIncludeHandler));
  }
  ...
  CHECK_HR(pCompiler->Compile(pTextBlob, fileName, L"", L"lib_6_3", nullptr, 0, nullptr, 0,
  ...
  IDxcBlob* pBlob;
  CHECK_HR(pResult->GetResult(&pBlob));
  return pBlob;
}
...
//
//
Microsoft::WRL::ComPtr<ID3D12DescriptorHeap> CreateDescriptorHeap(ID3D12Device* device, uint32_t count,
                                           D3D12_DESCRIPTOR_HEAP_TYPE type, bool shaderVisible)
{
  ...
  CHECK_HR(device->CreateDescriptorHeap(&desc, IID_PPV_ARGS(&pHeap)));
  return pHeap;
}
