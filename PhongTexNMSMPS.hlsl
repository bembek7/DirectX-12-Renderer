            ...
            float3 tangent : TANGENT, float3 bitangent : BITANGENT, float4 lightPerspectivePos : LIGHT_PERSPECTIVE_POSITION) : SV_TARGET
{
    ...
    const float3 specular = Speculate(specularMap.Sample(texSampler, texCoord).rgb, diffuseIntensity * specularIntensity, realViewNormal, lightVector.vectorToLight, viewPos, attenuation, roughness);
    ...
    return float4(texSample.rgb * (light + shadow), texSample.a);
}